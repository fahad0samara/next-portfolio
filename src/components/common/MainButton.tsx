...
const MainButton = forwardRef<HTMLButtonElement, MainButtonProps>(
  (
    ...
    const variant_hover =
      variant === "primary" ? "hover:bg-green-500 " : "hover:bg-secondary";

    return !isLoading ? (
      <Button
        form={form}
        className={`${
          isSecondaryVariant ? " text-white  bg-secondary" : "bg-green-500 "
      ...
      <Button
        className={`bg-green-500  text-white ${propWidth} md:${propWidth} select-none rounded-[0.625rem] cursor-not-allowed ${size_height} ${
          classes ? classes : ""
        }`}
        ref={ref}
        disabled
